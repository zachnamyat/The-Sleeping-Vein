...
- **Phase**: Phase 0 closed (all 21 seed tickets done per kanban migrations). Phase 1+ scaffolding extends through ~Phase 14 in code; awaiting playable verification + asset finalization.
...
- **Tests**: 21/21 GUT tests pass (`scripts/autoloads/save_system.gd`, combat math, health, manifest loader, skill xp). CI runs on push via `.github/workflows/test.yml`.
...
- **Manifest progress**: 6 / 76 sprite entries marked `final` (smoke_test_shaleseed, ui_game_icon, structure_chest, structure_bed, structure_loam_bench, structure_resonance_loom). The remaining 70 are `needed`.