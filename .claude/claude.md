...
- **Phase**: Phase 0/1/2/3 closed. Phase 3 implementation pass added equipment-slot validation, held-item visual on Walker, chest persistence in SaveSystem (save v3), crafting XP wiring, multi-craft + favorites + search, drag-to-drop + right-click context, stack split, sort/quick-stack/loot-all, placeable ghost-preview. Phase 4+ scaffolding extends in code; Phase 4 procedural world is the next critical-path phase.
...
- **Tests**: 50/50 GUT tests pass (combat math, health, manifest loader, skill xp, save system, inventory, loot table, crafting flow, equipment validation, chest persistence, stack split + sort). CI runs on push via `.github/workflows/test.yml`.
- **Autoloads**: 19 — gameplay (GameState, EventBus, SaveSystem, AudioBus, SkillSystem, ItemRegistry, Inventory, MiningSystem, CraftingSystem, NpcDirector, Compendium, BossDirector, NetSystem, FarmingSystem, Buffs, BiomeHazard, Achievements, FishingSystem, Housing) + infra (I18n, Settings).
- **Recent decisions**:
